...
const TeacherTickets = () => {
  ...
  const fetchTickets = async () => {
    try {
      ...
      // Fetch tickets from assigned students
      ...
      const formattedTickets: Ticket[] = (data || []).map((t: any) => ({
        ...
        studentName: studentNameMap[t.user_id] || "Unknown Student",
        studentId: t.user_id,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await sendNotification(
        "Ticket Update",
        `Mentor has responded to your ticket: ${selectedTicket.subject}`,
        "info",
  ...
  const handleUpdateStatus = async (newStatus: Ticket["status"]) => {
    ...
    try {
      ...
      await sendNotification(
        ...
        `Your ticket "${selectedTicket.subject}" status has been updated to ${newStatus}.`,
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Review and respond to your students' support tickets
            </p>
          </div>
          ...
          <div className="lg:col-span-1 space-y-3 max-h-[700px] overflow-y-auto pr-2">
            ...
            {filteredTickets.map((ticket, index) => (
              ...
              >
                ...
                <p className="text-xs text-primary font-medium mb-2">{ticket.studentName}</p>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
                        <span className="text-primary font-medium">{selectedTicket.studentName}</span>
                        <span>•</span>
                      ...
                      >
                        <div className="flex flex-col max-w-[75%]">
                          <span className={cn(
                            ...
                            {message.sender === "support" ? "You (Mentor)" : "Student"}
                          </span>
                          <div
                            className={cn(
